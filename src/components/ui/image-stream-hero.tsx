...
export type StreamImage = {
  ...
  alt?: string;
  /**
   * The image's own width ÷ height. Give it and the card takes that shape
   * instead of the uniform `cardWidth / cardHeight`, so a portrait shot rides
   * the corridor as a portrait and a landscape one as a landscape — nothing is
   * cropped to a house format.
   *
   * Card HEIGHT stays fixed and width varies. The other way round — fixed
   * width, varying height — makes the ribbon's top and bottom edges ragged at
   * every depth, and the corridor stops reading as one band.
   *
   * Supply it rather than measuring: the size has to be known before the image
   * loads or every card resizes as the network fills in. @default cardWidth / cardHeight
   */
  aspect?: number;
};
        ...
        <div className="absolute inset-0" style={{ transformStyle: "preserve-3d" }}>
          {[right, left].map((name) =>
            Array.from({ length: cards }, (_, i) => {
              ...
              // Height is the corridor's constant; width follows the image.
              const w = p.cardHeight * (img?.aspect ?? p.cardWidth / p.cardHeight);
              return (
                <div
                  key={`${name}-${i}`}
