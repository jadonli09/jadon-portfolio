...
import blurWgsl from "./shaders/blur";
import { BLUE_NOISE_SIZE, blueNoiseBytes } from "./blue-noise-128";
import compositeWgsl from "./shaders/composite";
import logoWgsl from "./shaders/logo";
import rimWgsl from "./shaders/rim";
...
/**
 * The example lights a pure black stage, where a little radiance goes a long
 * way. Screened over a bright photograph the same values barely register, so
 * the flare is given headroom. This is the one knob to turn for more or less
 * presence — `resolveDarkColor` saturates, so large values bloom rather than clip.
 */
const FLARE_GAIN = 1.55;
...
export class FlarePipeline {
  ...
  constructor(private readonly gpu: Gpu, private readonly output: Target) {
    ...
    try {
      this.effects = {
        logo: effect(gpu, fullscreen(logoWgsl), { label: "name-flare-logo" }),
        rim: effect(gpu, fullscreen(rimWgsl), { label: "name-flare-rim" }),
        rimBlurH: effect(gpu, fullscreen(blurWgsl), {
          label: "name-flare-rim-horizontal",
        }),
        rimBlurV: effect(gpu, fullscreen(blurWgsl), {
          label: "name-flare-rim-vertical",
        }),
        composite: effect(gpu, fullscreen(compositeWgsl), {
          label: "name-flare-composite",
        }),
      };
    } catch (error) {
  ...
  setFrameUniforms(
    ...
    this.effects.composite.set({
      params: {
        ...
        rimIntensity: attenuation * intensity * FLARE_GAIN,
        extension: 0.6,
        beamIntensity: 0.8 * attenuation * intensity * FLARE_GAIN,
        filmGrain: 0.03,
        smoothness: 1,
        logoOpacity: 0,
        frameIndex,
...
/**
 * Upstream placed a fixed-aspect logo in the middle of the canvas. Here the
 * glyph box is wherever the headline actually landed, so the placement is
 * derived from a measured box in backing pixels instead.
 */
export function boxPlacement(
  ...
  return {
    ...
    logoScale: [box.width / width, box.height / height],
...
function createBlueNoiseTexture(gpu: Gpu): GPUTexture {
  const texture = gpu.gpu.createTexture({
    label: "name-flare-blue-noise-128",
