// Verbatim rim.wgsl from the vgpu "nextjs-flare" example.
// Inlined as a TS string because Next/Turbopack has no .wgsl loader (vgpu ships a
// Vite plugin only). These shaders declare no WGSL imports, so raw source is enough.
const shader = `struct Params { light: vec2f, sceneTexel: vec2f, aspect: vec2f, spotReach: f32, spotStroke: f32 }
